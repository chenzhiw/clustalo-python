...
clustalo_clustalo(PyObject *self, PyObject *args, PyObject *keywds)
{
    ...
    PyObject *pItem;
    Py_ssize_t i , n;
	n = PyList_Size(inputList);
    for (i = 0; i < n; i++) {
		pItem = PyList_GetItem(inputList, i);
        char *seq = PyString_AsString(pItem);
        // Sanitize sequence.
        int seqPos;
    ...
    if (prMSeq->nseqs <= 1) {
        return PyList_AsTuple(inputList);
    }
    ...
    // Return the aligned results in a tuple.
...
static PyMethodDef ClustaloMethods[] = {
    {"clustalo",  (PyCFunction)clustalo_clustalo, METH_VARARGS | METH_KEYWORDS,
     ...
     ""
     "Returns list of aligned sequences ('_' for gaps)"},
    {NULL, NULL, 0, NULL}
};
